...
pub use super::rings::{ZZ12, ZZ4, ZZ8};
...
// NOTE: ZZ4, ZZ8 and ZZ12 are **not** generated by these macros. They
// live in `src/cyclotomic/rings.rs` on the new integer-basis storage.
// The rings listed here still use the legacy symbolic
// `GaussInt<Ratio>`-coefficient representation.
impl_symnum_struct_for!(10 16 20 24 32 60);
...
// `define_integral_zz!` for ZZ4 and ZZ8.
zz_triv_impl!(HasZZ4Impl, ZZ16 ZZ20 ZZ24 ZZ32 ZZ60);
impl_from_int_pair!(16 20 24 32 60);
...
// HasZZ8Impl is added directly via the `has:` clause for ZZ8 in `rings.rs`.
zz_triv_impl!(HasZZ8Impl, ZZ16 ZZ24 ZZ32);
...
mod tests {
    ...
    // NOTE: ZZ4, ZZ8 and ZZ12 are excluded -- they have a non-symbolic
    // integer-basis storage layout, so the generic SymNum-coefficient tests
    // (e.g. `test_scaling_fac`) don't apply. Their tests run via
    // `zz_integral_ring_tests!` in `rings.rs`.
    zz_tests!(zz, ZZ, 10 16 20 24 32 60);
}
