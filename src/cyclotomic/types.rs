...
use super::params::*;
use super::sign::{
    coeffs_real_sign_4_pentagonal, coeffs_real_sign_4_sym, coeffs_real_sign_fallback,
};
...
use super::traits::{
    ...
    IsRingOrField, RingTraits, ZZType,
};
...
pub use super::rings::{ZZ12, ZZ24, ZZ4, ZZ8};
...
// NOTE: ZZ4, ZZ8, ZZ12 and ZZ24 are **not** generated by these macros.
// They live in `src/cyclotomic/rings.rs` on the new integer-basis storage.
// The rings listed here still use the legacy symbolic
// `GaussInt<Ratio>`-coefficient representation.
impl_symnum_struct_for!(10 16 20 32 60);
...
// `define_integral_zz!` for ZZ4, ZZ8 and ZZ24.
zz_triv_impl!(HasZZ4Impl, ZZ16 ZZ20 ZZ32 ZZ60);
impl_from_int_pair!(16 20 32 60);
...
// HasZZ6Impl, HasZZ8Impl and HasZZ12Impl for ZZ24 are added directly via
// the `has:` clause in `define_integral_zz!`.
zz_triv_impl!(HasZZ6Impl, ZZ60);
zz_triv_impl!(HasZZ8Impl, ZZ16 ZZ32);
zz_triv_impl!(HasZZ10Impl, ZZ10 ZZ20 ZZ60);
zz_triv_impl!(HasZZ12Impl, ZZ60);
...
mod tests {
    ...
    // NOTE: ZZ4, ZZ8, ZZ12 and ZZ24 are excluded -- they have a
    // non-symbolic integer-basis storage layout, so the generic
    // SymNum-coefficient tests (e.g. `test_scaling_fac`) don't apply.
    // Their tests run via `zz_integral_ring_tests!` in `rings.rs`.
    zz_tests!(zz, ZZ, 10 16 20 32 60);
}
