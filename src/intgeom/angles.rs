...
pub fn unit_idx<T: IsRing>(angle: i8) -> i8 {
...
pub fn to_abs_seq<T: IsRing>(angles: &[i8]) -> Vec<i8> {
...
/// representation.
pub fn normalize_angle<T: IsRing>(angle: i8) -> i8 {
    let a = angle % T::turn();
    if a.abs() <= T::hturn() {
        a
    } else {
...
pub fn upscale_angles<T: IsRing>(src_ring: i8, angles: &[i8]) -> Vec<i8> {
