...
| V8  | Medium   | observability | done | removed unconditional `eprintln!` progress logs from `seed_phase` / `bfs_phase`. They were one-off debug helpers; spectre BFS runs in <1s and doesn't need progress indication |
| V9  | Medium   | smell     | done  | dropped 3 `#[allow(dead_code)]` fields (`has_transitions`, `successors`, `predecessors`) from `OpenVertexTypeInfo`. Their inputs (`pred_sets`, `transition_map`) were threaded through `classify_and_finalize` purely to populate fields with no consumers; both now removed too |
| V10 | Medium   | refactor  | done  | removed `transition_map` entirely from `BfsState` and the BFS loop. After V9 dropped its only consumer (`has_transitions`), the skip at the top of `bfs_phase` (`transition_map.get(&vt) == Some(&No)`) became dead (the `visited.insert` gate already prevents re-pushing). Dropped associated `HasTransitions` enum |
| V11 | Medium   | smell     | done  | `bfs_phase` now uses `witness_store.entry(...).or_insert_with(...)` to match the seed phase's first-seen-wins semantics |
| V12 | Medium   | memory    | deferred | drop grid in `witness_store` after BFS. Non-trivial refactor of `GrowingPatch` API (grid → `Option`, or store `RawBoundary` + lazy reconstruction in `witness()` accessor). Review doc itself flagged this as "Not urgent for current workloads"; deferring until a profile shows it's a bottleneck |
| V13 | Low      | tests     | done     | added hex/square/spectre validity + completeness tests. Counts after V2b + V2c fixes: 280 spectre types (102 alive: 82 Blessed + 20 Free, 178 cursed: 159 Dead + 19 Undead, 420 transitions). Caught V2b and V2c — both fixed. |
| V2b | Critical | correctness | done | `compute_cursed` ignored closing transitions; a VT with closing transitions + open transitions to cursed VTs was wrongly marked Undead. Added `has_closing[i]` short-circuit: cursed = attractor of Dead, treating closings as edges to the non-cursed Closed sink. |
| V2c | Critical | correctness | done | **`cyclic_range_contains` wrap-around bug at `start + len == n` exactly.** The function checked `index >= start && index <= end` on the non-wrap branch (`end <= n`), which fails for `index = 0` when `end = n` — but geometrically the match wraps to vertex 0. Replaced with `(index + n - start%n) % n <= len`. Latent because the only existing cross-check (`get_matches_touching_vertex_matches_brute_force_on_spectre`) filtered *both* sides with the same buggy function, masking the bug. Impact on spectre VT BFS: discovered **51 additional VT types** and **92 additional transitions** that were previously unreachable. Added regression tests: `cyclic_range_contains_unit` (pure unit) and `vt_witness_touching_matches_match_brute` (end-to-end on every VT witness). Also made the existing cross-check non-circular by replacing its brute-side filter with an independent modular check. |
| V2d | Critical | docs+assert | done | `glue_raw_angles` undocumented precondition: `(start_a, mlen, start_b)` must describe a real match, not just any tuple. Added doc + `debug_assert!` verifying the revcomp relation on the `mlen-1` interior angles. Propagated precondition docs to `compute_glue_angles`, `glue_match_to_raw_boundary`, and `add_tile`. |
| V14 | Low      | style     | done     | already addressed during V4 refactor; fixpoint loops use `for i in 0..n` directly |
| V15 | Low      | docs      | done     | doc-commented `OpenVertexTypeIndex` public methods (`num_types`, `transitions`, `entries`, `range_by_cw`, `get_id`, `get_type`, `get_info`, `tileset`), `OpenVertexTypeInfo` accessors, `TransitionInfo` field-level docs, and `CLOSED_ID` sentinel |
| V16 | Low      | docs      | done     | `range_by_cw`'s doc now states the entry-sort invariant explicitly (entries in `(cw, inner, ccw)` lex order; `cw.tile_id` is most-significant key) |
