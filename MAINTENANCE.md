...
*.py` -- baked into `rat_enum` via `include_str!` and written into every
dataset's `tools/` at emit time (`reproduce.sh` is generated by
`rocrate.rs`). Edit them in `extras/`, never the generated copies inside
a dataset. Run from a dataset directory (the asset dir, i.e. the
`--mode dafsa-blocks` output or `<D>/dafsa`):
